...
const InputGroup: React.FC<Props> = ({ isLoading }) => {
  ...
  const { setDataChat, focus: isFocus } = useSQRAI();
  const editableDivRef = useRef<HTMLDivElement>(null);
  // const [value, setValue] = useState<string>("");

  useEffect(() => {
    if (isFocus && editableDivRef.current) {
  ...
  return (
    <div className={`w-full transition-all border-t border-[#a4fb0e]`}>
      <div className="bg-black w-full flex items-center overflow-hidden pl-11 py-4 relative">
        <div className="text-[#a4fb0e] text-sm font-medium font-bricolage leading-[27px] flex absolute left-5 top-3">&gt;_</div>
        <div
