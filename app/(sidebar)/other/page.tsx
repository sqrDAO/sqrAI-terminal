...
const Overview = () => {
  ...
  const [knowledgeLinks, setKnowledgeLinks] = useState(
    JSON.parse(localStorage.getItem("selectedAgent"))?.knowledgeLinks || []
  );
  ...
  useEffect(() => {
    ...
    const updatedAgentList = agentList.map((agent) =>
      agent?.name === selectedAgent?.name ? updatedAgent : agent
    );
    ...
    <div className=" h-[calc(100vh_-77px)] overflow-auto w-full px-6 pt-6 flex-col justify-start items-center inline-flex">
        ...
        <div className="self-stretch justify-start items-center gap-2.5 inline-flex">
          <div className="grow shrink basis-0 flex-col justify-start items-start gap-1.5 inline-flex">
            <div className="self-stretch text-white text-3xl font-semibold font-chakra leading-[37.50px]">
              More Knowledge <span className="text-[10px] font-chakra text-[#A4FB0E]">(coming soon)</span>
            </div>
      ...
      <div className="h-fit flex-col justify-start items-start gap-8 inline-flex">
        <div className="self-stretch h-fit py-5 bg-black border-2 border-[#dcff9f] flex-col justify-center items-start gap-5 flex">
          ...
          >
            <div
              className={`grow shrink basis-0 px-4 py-8 border border-[#dcff9f] flex-col justify-center items-center gap-4 inline-flex ${
                isDragging ? "bg-[#444444]" : ""
              }`}
            >
          ...
          <div className="self-stretch h-fit flex-col justify-start items-start flex">
            ...
            {knowledgeLinks.map((link, index) => {
              return (
                <div
                  key={index}
                  className="w-full px-5 py-4 border-b border-[#444444] grid grid-cols-5 gap-2.5"
                >
                  <div className="text-[#999999] text-sm font-semibold font-bricolage leading-tight">
                    {link?.name}
                  </div>
                  <div className="text-[#999999] text-sm font-semibold font-bricolage leading-tight">
                    {link?.size
                      ? (link?.size / 1024576)?.toFixed(2) + " MB"
                      : ""}
                  </div>
                  <div className="text-[#999999] text-sm font-semibold font-bricolage leading-tight">
                    {link?.type}
                  ...
                  <div className="text-[#999999] text-sm font-semibold font-bricolage leading-tight">
                    {dayjs(link?.addedAt).format("MMM DD, YYYY")}
                  </div>
